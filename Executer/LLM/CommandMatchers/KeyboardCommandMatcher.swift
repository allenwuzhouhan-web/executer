...
extension LocalCommandRouter {
    ...
    // Static lookup tables — avoid re-creating 25+ entry dictionaries on every call
    private static let keyPresses: [String: String] = [
    ...
    private static let hotkeys: [String: String] = [
    ...
    func tryKeyboardCommand(_ input: String) async -> String? {
        ...
        if let key = Self.keyPresses[input] {
            return try? await PressKeyTool().execute(arguments: "{\"key\": \"\(key)\"}")
        }

        if let combo = Self.hotkeys[input] {
